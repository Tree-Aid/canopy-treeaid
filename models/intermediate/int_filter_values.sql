...
union
select distinct country,project as project_code, year::DOUBLE PRECISION as assessment_year from {{ source('onadata', 'beneficiary_list') }}
)
select 
case when project_code like 'B%' then 'Burkina Faso' else country end as country,
project_code,
assessment_year 
from filter_values
